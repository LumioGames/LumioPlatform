...
public sealed class AccountWorld
{
    ...
    public bool TryGetByAccountId(string accountId, out AccountIdentityComponent component)
    {
        lock (identities)
        {
            if (byAccountId.TryGetValue(accountId, out var entityId) && identities.TryGetValue(entityId, out var found))
            {
                component = found;
                return true;
            }
