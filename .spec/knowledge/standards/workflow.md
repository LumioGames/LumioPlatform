...
- 公共架构变更不从本仓直接发起：先在架构仓 `LumioGameEngine` 完成 ADR 与 `engine/wire` 契约（`node eng/verify-wire.mjs` 通过并合入 `origin/main`），再同步本仓 `contract/` 只读镜像。
...
- 根 `.github/CODEOWNERS` 当前将全仓指派给 `@Go1c`；涉及鉴权、Network、Release 路由、维护、持久化或资源治理的改动必须由对应所有者审查。
- 合并前必须通过 Repository Policy、负载/故障矩阵中适用项、收口门槛与 reviewer 闭环；生产迁移和维护行为先说明影响与可逆性。
