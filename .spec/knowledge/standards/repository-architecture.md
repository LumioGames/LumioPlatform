...
| 接缝 | 方向 | 契约 |
| --- | --- | --- |
| 账号登录 / account-auth 签发（Bot 启动器、集成考卷、工具） | 客户端 → 平台 WS `/account`（子协议 `lumio-account-v1`） | `account-port-v1.json`；返回 unbound `accountAuthCredential`，不可直接入 Room |
| 网页注册 / 登录 / 会话 / 启动游戏 | 浏览器 → 平台 HTTP `/api/*` | `platform-port-v1.json` |
| 取服务器地址与 Room 凭证 | 浏览器 Cookie 或 WS account-auth Bearer → `POST /api/games/{slug}/launch` | `platform-port-v1.json` `launch`；调用方只提供 slug，allocator context 为服务端真值 |
| 凭证验票 | Game Server 进程内 `verify_admission(ticket, trustedContext)`（公钥 keyId 对应） | `account-port-v1.json`；LumioServer 原子升级并逐项校验六元绑定，拒绝 unbound WS credential |
...
- 端口默认只绑环回并动态分配；访问控制、CSRF、OTP HMAC、session epoch 与端点分区限流随各自功能卡交付，不得延后到 P5-2；生产暴露仍须通过最终门。
