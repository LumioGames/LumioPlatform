...
`LumioPlatform` 是 Lumio 游戏平台：**唯一账号权威**（邮箱注册、AccountId / UID / 用户名 / 头像、Argon2id 口令、Ed25519 准入凭证签发）、**游戏大厅**（展示已发布的自研游戏，浏览器内点开即联机）、**反馈**（bug / 建议 + 飞书 / QQ 群跳转）与**运营后台**（用户、登录记录、活跃度、埋点、反馈处理、游戏目录）。一个 ASP.NET Core 10 进程（Kestrel）同时承载 SPA 静态页、HTTP API、`lumio-account-v1` WebSocket 端口与 `/games/<slug>/` 静态游戏页；持久真值在 PostgreSQL。

- 公共语义（账号端口、准入凭证、launch 端口）的唯一真值在架构仓 `LumioGameEngine` 的 `engine/wire/*.json` 与 ADR；本仓只消费镜像（`contract/`），不另写协议。
- 本仓是 SDK consumer / Host：不实现 ECS、DS、Voxel 或玩法；Game Server 验票（`verify_admission`）在 `LumioServer`，游戏页在 `LumioClient`，集成考卷在 `LumioGame`。
- 开工前先读 [`repository-architecture.md`](knowledge/standards/repository-architecture.md) 与 [`features/platform.md`](knowledge/features/platform.md)；实施蓝图见 [`plans/`](plans/)。
