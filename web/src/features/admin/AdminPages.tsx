...
export function AdminPage() {
  return <Frame title="运营后台" description="平台运行概览与管理入口。"><div className={styles.sectionTitle}><h2>今日概览</h2><span className="ui-hint">当前为演示数据，接入 API 后自动更新</span></div><div className="ui-stats"><div className="ui-stat"><strong className="ui-stat__n">--</strong><span className="ui-stat__l">活跃用户</span></div><div className="ui-stat"><strong className="ui-stat__n">--</strong><span className="ui-stat__l">今日注册</span></div><div className="ui-stat"><strong className="ui-stat__n">--</strong><span className="ui-stat__l">游戏启动</span></div><div className="ui-stat"><strong className="ui-stat__n">--</strong><span className="ui-stat__l">待处理反馈</span></div></div><div className={styles.overviewGrid}><div className="ui-card"><div className={styles.cardHeading}><h3>房间实时人数</h3><span className="ui-pill ui-pill--soon">暂无数据</span></div><div className={styles.emptyMetric}>接入 /api/admin/stats 后显示</div></div><div className="ui-card"><div className={styles.cardHeading}><h3>分享统计</h3><span className="ui-pill ui-pill--soon">暂无数据</span></div><div className={styles.emptyMetric}>当前没有分享统计 API</div></div><div className="ui-card"><div className={styles.cardHeading}><h3>最近活动</h3><span className="ui-hint">最近 7 天</span></div><div className={styles.activityList}><span>平台基础设施已就绪</span><span>等待真实事件接入</span></div></div></div></Frame>;
}
...
function UsersPage() {
  ...
  const toggle = (uid: number, name = '该账号', status = 'active') => {
    if (status === 'active' && !window.confirm(`确认停用 ${name}？`)) return;
    setRows((current) => current.map((row) => row.uid === uid ? { ...row, status: row.status === 'active' ? 'banned' : 'active' } : row));
  };
