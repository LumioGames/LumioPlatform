      ...
      <section className={styles.section} id="games" aria-labelledby="games-title">
        ...
        <div className="ui-grid">
          {games.map((game, index) => game.status === 'published' ? (
            <article className={`ui-card ui-card--game ui-motion-enter ${index === 0 ? 'ui-card--pop' : ''}`} style={{ '--motion-delay': `${index * 80}ms` } as CSSProperties} key={game.slug}>
            ...
            <article className={`ui-card ui-card--game is-soon ui-motion-enter ${index === 1 ? styles.cardMint : styles.cardRose}`} style={{ '--motion-delay': `${index * 80}ms` } as CSSProperties} key={game.slug}>
